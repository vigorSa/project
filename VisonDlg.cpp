...
void CVisonDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ReporterList, m_RepoterList);
}
...
BEGIN_MESSAGE_MAP(CVisonDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_NOTEST, &CVisonDlg::OnBnClickedBtnNotest)
END_MESSAGE_MAP()
...
BOOL CVisonDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
    InitReporter();
	m_bitmap.SetBitmap(m_hIcon_OFF);
...
void CVisonDlg::InitReporter()
{	
	CRect rect;
    //获取编程语言列表视图控件的位置和大小
    m_RepoterList.GetClientRect(&rect);
    //为列表视图控件添加全行选中和栅格风格
    m_RepoterList.SetExtendedStyle(m_RepoterList.GetExtendedStyle() | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);  
    m_RepoterList.InsertColumn(0, _T("序号"), LVCFMT_CENTER, rect.Width()/6, 0);   
    m_RepoterList.InsertColumn(1, _T("孔直径"), LVCFMT_CENTER, rect.Width()/6, 1);    
	m_RepoterList.InsertColumn(2, _T("孔间距"), LVCFMT_CENTER, rect.Width()/6, 2); 
	m_RepoterList.InsertColumn(3, _T("弯曲角度"), LVCFMT_CENTER, rect.Width()/6, 3); 
	m_RepoterList.InsertColumn(4, _T("检测结果"), LVCFMT_CENTER, rect.Width()/6, 4);
	m_RepoterList.InsertColumn(5, _T("误差率"), LVCFMT_CENTER, rect.Width()/6, 5); 
	m_RepoterList.InsertItem(0,"模板");
	m_RepoterList.SetItemText(0,4,_T("/"));
	m_RepoterList.SetItemText(0,5,"/");
	m_RepoterList.InsertItem(1,"1");
	m_RepoterList.InsertItem(2,"2");
	m_RepoterList.InsertItem(3,"3");
...
void CVisonDlg::OnBnClickedBtnNotest()
{
	// TODO: 在此添加控件通知处理程序代码
	m_bitmap.SetBitmap(m_hIcon_OFF);
